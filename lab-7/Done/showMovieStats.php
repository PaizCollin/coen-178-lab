...
<head>
   ...
   <!-- Edit the link -->
   <a href='http://linux.students.engr.scu.edu/~cpaiz/showMovieStats.php?show=true'>Show Movie Stats</a>
   <br><br>
<?php
...
function showStats(){
	...
	$conn = oci_connect('cpaiz', '---', '//dbserver.engr.scu.edu/db11g');
	if (!$conn) {
		$e = oci_error();
	...
	oci_execute($query2);
	*/
	$query2 = oci_parse($conn, "SELECT movieName, COUNT(*) NoOfReviews FROM MovieReviews GROUP BY movieName HAVING COUNT(*) = (SELECT MAX(COUNT(*)) FROM MovieReviews GROUP BY movieName)");
	...
	while (($row = oci_fetch_array($query2, OCI_BOTH)) != false) {
		...
		// Use the uppercase column names for the associative array indices
		echo "</br>";
		echo "<font color=blue size='24pt'> Movie(s) with most no. of reviews </font>";
		echo "</br>";
		echo "</br>";
		echo "<font color='black'>Movie: </font>";
		echo "<font color='green'>  $row[0] </font>";
